...
export class Orchestrator extends Agent<Env, OrchestratorState> {
  ...
  async onMessage(connection: import("agents").Connection, message: string | ArrayBuffer | ArrayBufferView) {
    ...
    try {
      ...
      // Чат идёт через ту же возможность, что и вопрос через /api/task.
      // Раньше здесь был голый вызов модели: без памяти, без счёта, с
      // системным промптом «ты не выполняешь здесь задачи». Один и тот же
      // вопрос получал разные ответы в зависимости от того, каким входом
      // его задали, а посчитать в чате было нельзя вовсе.
      //
      // История передаётся дальше: без неё «а почему?» следом за вопросом
      ...
      replyText = answered.summary || "(пустой ответ модели)";
      const answerModel = (answered.data as { model?: string } | undefined)?.model;
      await addMessage(this.env, convId, "assistant", replyText, undefined, answerModel);
    } catch (err) {
  ...
  private async classifyIntent(request: TaskRequest): Promise<Intent> {
    ...
    /* Разбор по форме фразы — ДО модели.
     *
     * Глагол «создай» не предмет для рассуждения: там, где намерение
     * читается по форме, вызов модели добавляет только шанс ошибиться.
     * Именно здесь «напиши визитку» уезжала в qa_check — промпт ниже
     * описывал пять намерений и молчал про generate_code.
     *
     * Разбор узкий и отвечает только когда уверен; во всём спорном
     * возвращает null и отдаёт решение модели. */
    const quick = quickIntent(request.payload ?? "");
    if (quick) {
      log("info", "orchestrator.intent_by_form", { intent: quick });
      return quick;
    }
        ...
        messages: [
          {
            role: "system",
            content:
              "Классифицируй запрос ровно одним словом из списка: answer, analyze_spec, review_repo, generate_code, deploy, git_operation, generate_ui, security_scan, qa_check, evolution_audit, unclear. " +
              "answer — если это ВОПРОС, а не поручение: просят объяснить, рассказать, сравнить, посоветовать, посчитать, оценить. Признак: ответом служит текст, а не изменение в проекте. " +
              // ЭТОЙ СТРОКИ НЕ БЫЛО. Промпт описывал пять намерений и
              // молчал про generate_code — самое частое поручение вообще.
              // Модель выбирала из растолкованного, и «создай визитку»
              // уходила в qa_check, где агент искал несуществующие модули.
              "generate_code — если просят СОЗДАТЬ, написать, сделать, исправить или дополнить код, файлы, скрипт, проект. Признак: результатом служит изменение в файлах. Это самое частое поручение — выбирай его, когда просят сделать что-то в коде и никакая более узкая метка не подходит. " +
              "generate_ui — если просят интерфейс, компонент, экран, вёрстку. security_scan — если просят проверить безопасность, уязвимости, утечки ключей, зависимости. qa_check — если просят про тесты, покрытие, прогон CI, качество. evolution_audit — если просят аудит проекта целиком, что улучшить, что устарело, где узкие места. " +
              "unclear — только если запрос непонятен настолько, что даже тип неясен. Вопрос, на который есть ответ, — это answer, а не unclear. " +
              "Ответь только этим словом, без пояснений.",
          },
